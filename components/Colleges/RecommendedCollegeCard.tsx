...
const RecommendedCollegeCard = ({ college, onPress, hideFav }: CollegeProps) => {
  ...
  // Fixed card width calculation - more consistent sizing
  const getCardWidth = () => {
    if (isTablet) {
      return (windowWidth - 64) / 2.5; // 2.5 columns on tablets to make cards more visible
    } else if (isLargePhone) {
      return (windowWidth - 58) / 1.8; // Slightly wider cards on large phones
    }
    return windowWidth * 0.75; // Fixed width on small phones that shows part of next card
  };
    ...
    <View style={[styles.cardContainer, { width: cardWidth }]}>
      ...
      >
        ...
        <View style={styles.contentContainer}>
          <View style={styles.instituteNameContainer}>
...
export default React.memo(RecommendedCollegeCard);
...
const styles = StyleSheet.create({
  ...
  gradientOverlay: {
    ...
    bottom: 0,
    height: '70%',
    backgroundColor: 'rgba(0,0,0,0.0)', // Slightly darkened for better text visibility
  },
  overlay: {
    position: 'absolute',
    bottom: 12,
  ...
  instituteNameContainer: {
    height: 48, // Fixed height for name
    marginBottom: 8,
  },
  locationPill: {
    flexDirection: 'row',
